...
VertexArray::VertexArray(const float* verts, unsigned int numVerts,
	...
	glBufferData(
		...
		(unsigned long long)(numVerts) * 8 * sizeof(float),  // number of bytes to copy - each vertex has 3 floats for its 3D position, 2 floats for texture coord, and 3 floats for RGB
	...
	// enable the second vertex attribute (attribute 1) for texture coords
	glEnableVertexAttribArray(1);
	glVertexAttribPointer(
		1,  // vertex attribute index
		2,  // number of components (2 because u, v coord)
	...
	// enable the third vertex attribute (attribute 2) for vertex colours - Exercise 5.2
	glEnableVertexAttribArray(2);
	glVertexAttribPointer(
		2,
		3,  // 3 components for R, G, B values
		GL_FLOAT,
		GL_FALSE,
		sizeof(float) * 8,
