...
void Game::LoadData() {
    ...
    a->SetScale(1.0f);
    mc = new MeshComponent(a, mRenderer->GetMesh("Assets/Sphere.gpmesh"));
    mSphereMove = new MoveComponent(a);
    mSphereMove->SetForwardSpeed(400.0f);
...
void Game::HandleKeyPress(int key) {
    ...
    case 'r':
        ...
        if (!mReverbSnapshot.IsValid()) {
            mReverbSnapshot = mAudioSystem->PlayEvent("snapshot:/WithReverb");
        }
        else {
            mReverbSnapshot.Stop();
        }
        break;
    case '1':
...
void Game::UpdateGame() {
    ...
    // update oscillating sphere
    mOscillateTimer -= deltaTime;
    if (mOscillateTimer <= 0.0f) {
        mSphereMove->SetForwardSpeed(-mSphereMove->GetForwardSpeed());
        mOscillateTimer = 3.0f;
    }
