...
Grid::Grid(class Game* game) : Actor(game) {
	...
	for (size_t i = 0; i < NumRows; ++i) {
		for (size_t j = 0; j < NumCols; ++j) {
			...
			mTiles[i][j]->SetPosition(Vector2(TileSize / 2.0f + j * TileSize, i * TileSize + StartY));
		}
	}
...
void Grid::ProcessClick(int x, int y) {
	// offset y position so it starts from top edge of screen (y = 0) rather than from StartY
	y -= static_cast<int>(StartY - TileSize / 2);
	if (y >= 0) {
		// make sure the mouse has clicked within the vertical range of tiles location
		// if so, convert the x,y positions into indices into the mTiles array
...
void Grid::BuildTower() {
	if (mSelectedTile && !mSelectedTile->mBlocked) {
		...
		if (FindPath(GetEndTile(), GetStartTile())) {
			// a path from start to end tile still exists, allow player to build tower here
