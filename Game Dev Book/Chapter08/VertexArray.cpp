...
VertexArray::VertexArray(const float* verts, unsigned int numVerts,
	...
	glBufferData(
		...
		(unsigned long long)(numVerts) * 5 * sizeof(float),  // number of bytes to copy - each vertex has 3 floats for its 3D position, 3 floats for its normal, and 2 floats for its texture coord
	...
	glVertexAttribPointer(
		...
		sizeof(float) * 5,  // stride - byte offset between consecutive vertices' attributes (usually size of each vertex)
	...
	glVertexAttribPointer(
		...
		2,  // number of components (3 for normal xyz)
